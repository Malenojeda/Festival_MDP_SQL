...
--  1 Cantidad de pelicula por competencia
...
-- 2. Ordenar peliculas segun duración
create view vw_duracion as
select  Nombre, Duracion
from pelicula
order by Duracion desc;

-- 3  con Nombre y apellido de Jurado y la competencia
...
-- 4. Agrupar  y sumar peliculas por ID_pais
create view vw_pelicula_pais as
select ID_pais , count(*) as total
from pelicula
group by ID_pais;

-- 5. Vista con el nombre del pais a la suma de peliculas por pais
...
-- Funcion para poder saber el nombre de la pelicula a traces del ID
DELIMITER //
create function  nombre_pelicula(id int) returns varchar (50)
deterministic
begin
...
-- 1
-- Tabla bitacora para tabla COMPETENCIAS
create table bitacora_dato_competencia(
ID_bit int not null primary key auto_increment,
...
-- Trigger insercion // Before
create trigger insert_log_comp
before insert on competencias
for each row
insert into bitacora_dato_competencia (fecha, ID_usuario, new_dato, operacion)
values
(now(),session_user(), new.competencia, 'Nuevo dato');

-- Ejemplo
	-- insert into competencias (competencia) values ('Competencia prueba');
	-- select * from bitacora_dato_competencia;

-- Trigger Actualización // After
create trigger update_log_comp
after update on competencias
for each row
insert into bitacora_dato_competencia (fecha, ID_usuario, old_dato, new_dato, operacion)
values
(now(),session_user(), old.competencia, new.competencia, 'Actualizacion de dato');

-- Ejemplo
	-- update competencias set competencia = 'Competencia prueba2' where ID_competencias= 7;
	-- select * from bitacora_dato_competencia;

-- 2
-- Tabla bitacora para tabla JURADOS
create table bitacora_dato_jurados(
ID_bit int not null primary key auto_increment,
fecha datetime not null,
ID_usuario varchar (50),
old_nombre_apellido varchar(50),
new_nombre_apellido varchar(50),
old_id_competencia int,
new_id_competencia int,
operacion varchar (50)
);

-- Trigger insercion // After
create trigger insert_log_jur
after insert on jurados
for each row
insert into bitacora_dato_jurados (fecha, ID_usuario, new_nombre_apellido, new_id_competencia, operacion)
values
(now(),session_user(), new.nombre_apellido, new.ID_competencias, 'Nuevo dato');

-- Ejemplo
	-- insert into jurados (nombre_apellido, ID_competencias) values ('Juan Perez', 3);
	-- select * from bitacora_dato_jurados;

-- Trigger Delete // before
create trigger delete_log_jur
before delete on jurados
for each row
insert into bitacora_dato_jurados (fecha, ID_usuario, old_nombre_apellido, old_id_competencia, operacion)
values
(now(),session_user(), old.nombre_apellido, old.ID_competencias, 'Dato borrado');

-- Ejemplo
	-- delete from jurados where ID_jurados= 15;
	-- select * from bitacora_dato_jurados;
    
use mysql;
-- 1. Usuario 1
-- Creacion usuario 
create user 'User01'@'localhost' identified by 'MDPFestival01';

-- Otorgacion permisos SOLO LECTURA de todas las tablas

grant select on *.* to 'User01'@'localhost';

-- 2. Usuario 2
-- Creacion usuario 
create user 'User02'@'localhost' identified by 'MDPFestival02';

-- Otorgacion permisos LECTURA, INSERCION Y MODIFICACION  de todas las tablas

grant select, insert, update on *.* to 'User02'@'localhost';
...
-- 1.  Eliminar registros de tabla // Rollback // Commit
start transaction;
delete from jurados where ID_competencias = 4;
...
-- 2.Insercion de registros //Savepoint
start transaction;
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Dinamarca');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Colombia');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Venezuela');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Cuba');
-- Ejecucion Savepoint 1
savepoint SP_pais_1;
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Suecia');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Noruega');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Polonia');
INSERT INTO pais (`ID_pais`,`pais`) VALUES (null,'Republica Checa');
-- Ejecucion Savepoint 2
savepoint SP_pais_2;

-- Eliminacion Savepoint 1
release savepoint SP_pais_1;
